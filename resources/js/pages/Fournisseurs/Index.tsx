...
export default function Index({ fournisseurs }: IndexProps) {
    ...
    // Filter fournisseurs based on the search query
    const filteredFournisseurs = fournisseurs.filter(fournisseur =>
        fournisseur.nom_fourn.toLowerCase().includes(searchQuery.toLowerCase()) ||
        fournisseur.adresse_fourn.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                        <div className="p-8">
                            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
                                <h1 className="text-3xl font-extrabold text-gray-900">Gestion des Fournisseurs</h1>
                                <Link
                            ...
                            <div className="overflow-x-auto rounded-lg shadow-xl border border-gray-200">
                                <table className="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wider rounded-tl-lg">Nom</th>
                                    ...
                                    {filteredFournisseurs.length > 0 ? (
                                        filteredFournisseurs.map((fournisseur, index) => (
                                            <tr key={fournisseur.nom_fourn} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-100 transition-colors duration-200 ease-in-out`}>
                                        ...
                                        <tr>
                                            <td colSpan={4} className="px-6 py-10 text-center text-xl text-gray-500 font-medium">
                                                Aucun fournisseur trouvé. Veuillez en ajouter un pour commencer !
                                            </td>
                                        </tr>
                                    )}
                                    </tbody>
