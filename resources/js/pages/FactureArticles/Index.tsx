...
// Define the Article interface for nested data
interface Article {
    nom_art: string;
    marque_art: string;
...
// Define the FactureArticle interface, including nested Client and Article
interface FactureArticle {
    ...
    article: Article | null; // Facture can have an article or not
                    ...
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                        <div className="p-8"> {/* Increased padding */}
                            ...
                            <div className="flex justify-between items-center mb-8">
                                <h1 className="text-3xl font-extrabold text-gray-900">Facture Articles Management</h1>
                                <Link
                                    ...
                                    {factures.length > 0 ? (
                                        factures.map((facture, index) => (
                                            <tr key={facture.id_facture_art} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-100 transition-colors duration-200 ease-in-out`}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                    ...
                                                    <Link
                                                        method="delete"
                                                        href={route('facture-articles.destroy', facture.id_facture_art)}
                                        ...
                                        <tr>
                                            <td colSpan={6} className="px-6 py-10 text-center text-xl text-gray-500 font-medium">
                                                No facture articles found. Please add a new one to get started!
                                            </td>
                                        </tr>
                                    )}
                                    </tbody>
