...
export default function Index({ reparateurs }: IndexProps) {
    ...
    // Filter reparateurs based on the search query
    const filteredReparateurs = reparateurs.filter(reparateur =>
        reparateur.nom_rep.toLowerCase().includes(searchQuery.toLowerCase()) ||
        reparateur.num_tel_rep.toLowerCase().includes(searchQuery.toLowerCase()) ||
        reparateur.adresse_rep.toLowerCase().includes(searchQuery.toLowerCase())
    );

    return (
        <AppLayout>
            <Head title="Gestion des Réparateurs" />
            <div className="min-h-screen bg-gray-100 py-12">
                <div className="w-full mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                        <div className="p-8">
                            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
                                <h1 className="text-3xl font-extrabold text-gray-900">Gestion des Réparateurs</h1>
                                <Link
                            ...
                            <div className="overflow-x-auto rounded-lg shadow-xl border border-gray-200">
                                <table className="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wider rounded-tl-lg">Nom du Réparateur</th>
                                    ...
                                    {filteredReparateurs.length > 0 ? (
                                        ...
                                        <tr>
                                            <td colSpan={4} className="px-6 py-10 text-center text-xl text-gray-500 font-medium">
                                                Aucun réparateur trouvé. Veuillez en ajouter un pour commencer !
                                            </td>
                                        </tr>
                                    )}
                                    </tbody>
